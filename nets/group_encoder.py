...
class GroupEncoder(nn.Module):
    def __init__(self, n_heads, embed_dim, feed_forward_hidden=512):
        super().__init__()
        self.encoder = EncoderLayer(n_heads, embed_dim, feed_forward_hidden)

    def forward(self, h, mask, distance=None):
        h = self.encoder(h, mask, distance)
        return h
...
class EncoderLayer(nn.Module):
    ...
    def forward(self, input1, mask, distance=None, multiplier=None):
        ...
        out_concat = self.multi_head_attention(q, k, v, mask, distance, multiplier)
    ...
    def multi_head_attention(self, q, k, v, mask=None, distance = None, multiplier=None, rank2_ninf_mask=None, rank3_ninf_mask=None):
        batch_s = q.size(0)
        head_num = q.size(1)
        n = q.size(2)
        key_dim = q.size(3)
        ...
        # if distance is not None:
            cost_mat_score = distance[:, None, :, :].expand_as(score_scaled)
            two_scores = torch.stack((score_scaled, cost_mat_score), dim=4)
            ...
            score_scaled = mixed_scores.squeeze(4)
