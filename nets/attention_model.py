...
from tsp_solver.nets.graph_encoder import GraphAttentionEncoder
...
class AttentionModel(nn.Module):
    ...
    def forward(self, input, return_pi=False):
        ...
        if self.checkpoint_encoder and self.training:  # Only checkpoint if we need gradients
            ...
            embeddings = self.embedder2(self._init_embed(input["loc"]), input["mask"])
    ...
    def _inner(self, input, embeddings):
        ...
        fixed = self._precompute(embeddings, input["mask"])
    ...
    def sample_many(self, input, batch_rep=1, iter_rep=1):
        ...
        #     (input, self.embedder2(self._init_embed(input["loc"]))[0], input["mask"]),  # Pack input with embeddings (additional input)
        #     batch_rep, iter_rep
        # )
    ...
    def _precompute(self, embeddings, mask=None, num_steps=1):
        graph_embed = embeddings.clone()
        graph_embed[mask] = torch.tensor(float('nan'), device=embeddings.device)
        # The fixed context projection of the graph embedding is calculated only once for efficiency
        graph_embed = graph_embed.nanmean(1)
    ...
    def _get_parallel_step_context(self, embeddings, state, from_depot=False):
        ...
        if self.is_vrp:
            ...
            if num_steps == 1:  # We need to special case if we have only 1 step, may be the first or not
                ...
                #     first_nodes = torch.zeros(batch_size, 1, embeddings.size(-1), device=embeddings.device)
    ...
    def _one_to_many_logits(self, query, glimpse_K, glimpse_V, logit_K, mask):
        ...
        s = torch.softmax(compatibility, dim=-1)
