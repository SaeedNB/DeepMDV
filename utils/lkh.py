...
def solve_lkh_log(executable, directory, name, loc, runs=1, disable_cache=False):
    ...
    try:
        ...
        if os.path.isfile(output_filename) and not disable_cache:
            ...
            params = {"PROBLEM_FILE": problem_filename, "OUTPUT_TOUR_FILE": tour_filename, "RUNS": runs, "SEED": 1234, "MAX_TRIALS": 1}
...
def lkh_solve(opts, dataset, instance_idx):
    dataset_basename = f'cvrp{100}'
    results_dir = os.path.join("results", "subtsp", dataset_basename)
    ...
    # _dataset = [(instance.numpy().tolist(),) for instance in dataset]
    _dataset = [(instance,) for instance in dataset]
    use_multiprocessing = False
    executable = get_lkh_executable()

    def run_func(args):
        return solve_lkh_log(executable, *args, runs=1, disable_cache=False)

    results, parallelism = run_all_in_pool(
        run_func,
        target_dir, _dataset, opts, use_multiprocessing=use_multiprocessing
    )
    filtered_list = [t for t in results if all(item is not None for item in t)]
    costs, tours, durations = zip(*filtered_list)
